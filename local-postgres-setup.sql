-- =============================================================================
-- BRD Loan CRM — LOCAL DEVELOPMENT PostgreSQL Setup
-- Run as superuser: psql -U postgres -f local-postgres-setup.sql
--
-- This script creates the local DB and brdapp user with full permissions.
-- =============================================================================
...
-- Step 4: Grant connect
GRANT CONNECT ON DATABASE loancrm TO brdapp;

-- Step 5: Connect to loancrm and grant schema/table permissions
\c loancrm
...
-- Step 6: Default privileges for future tables/sequences Django will create
...
SELECT
  ...
  rolcreatedb,
  rolcanlogin
FROM pg_roles WHERE rolname = 'brdapp';

SELECT 'SUCCESS: brdapp user configured on local loancrm database.' AS result;
