...
import { IconButton } from "../../../components/master/Controls/SharedUIHelpers";
...
export default function FundManagement() {
  ...
  /* ---------------- SEARCH FILTER ---------------- */
  const filteredFunds = (Array.isArray(funds) ? funds : []).filter(
    (f) =>
  ...
  if (loading) return <>Loading fund data...</>;
      ...
      <div className="space-y-3">
        ...
        {filteredFunds.map((fund) => (
          ...
          >
            ...
            {/* Status */}
            <span
              className={`px-3 py-1 text-xs rounded-full justify-self-start ${fund.status === "Active"
