  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const allBanks = Array.isArray(allBanksRes) ? allBanksRes : (allBanksRes?.results || []);
        const portfolioData = Array.isArray(portfolioRes) ? portfolioRes : (portfolioRes?.results || []);
        ...
        const portfoliosWithBankObjects = portfolioData.map((p) => ({
          ...p,
          banks: (Array.isArray(p.banks) ? p.banks : []).map((bId) =>
        ...
        <div className="space-y-3">
          ...
          {filteredPortfolios.map((p) => (
            ...
            >
              ...
              <span
                className={`px-3 py-1 text-xs rounded-full justify-self-start ${p.is_active
